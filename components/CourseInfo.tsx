...
function CourseInfo(props: CourseInfoProps) {
  ...
  const archieveMutation = useMutation({
    ...
    mutationFn: async () => {
      const { data } = await axiosInstance.patch(`/courses/${course.id}/archieved`, {
  ...
  const liveLinkMutation = useMutation({
    ...
    mutationFn: async () => {
      const { data } = await axiosInstance.patch(`/courses/${course.id}/live-link`, {
    ...
    onSuccess: (data) => {
      ...
      // Clear live link input
      if (liveLinkInputRef.current) {
        liveLinkInputRef.current.value = '';
      }
    },
  });
  ...
  return (
    <div>
          ...
          {course.archived ? (
            <div className="alert alert-warning rounded-none shadow-lg">
              <div>
                ...
                <span>
                  Course is archived. You can unarchive it to make it available to students.
                </span>
              </div>
            </div>
          ) : null}
      ...
      <Protected permissions={'update:course'}>
        <div className="mt-4 ">
          <Link to={`/admin/courses/edit/${course.id}`} className="link link-primary">
